...
ES_Event RunCanRX_Service( ES_Event ThisEvent )
{
  ...
  switch(ThisEvent.EventType) {
      case ES_INIT:
        ...
        case IDLE_MESSAGE:
            SetPattern(IDLE_PATTERN);
            ES_Event PatternEvent;
            PatternEvent.EventType = PATTERN_START;
            PostPatternControlService(PatternEvent);
            break;
            
        case THINKING_MESSAGE:
            SetPattern(THINKING_PATTERN);
            ES_Event PatternEvent;
            PatternEvent.EventType = PATTERN_START;
            PostPatternControlService(PatternEvent);
            break;
            
        case SPEAKING_MESSAGE:
            SetPattern(SPEAKING_PATTERN);
            ES_Event PatternEvent;
            PatternEvent.EventType = PATTERN_START;
            PostPatternControlService(PatternEvent);
            break;
...
static void ProcessReceive(void)
{
    ...
    if (RXB0SIDH == SIDH && RXB0SIDL == SIDL && RXB0EIDH == EIDH && RXB0EIDL == EIDL)  {
        ...
        switch(RXB0D0) { //Read First Data Byte
            case IDLE:
                Event2Post.EventType = IDLE_MESSAGE;
                break;
            case SPEAKING:
                Event2Post.EventType = SPEAKING_MESSAGE;
                break;
            case LOADING:
                Event2Post.EventType = THINKING_MESSAGE;
                break;
            case LISTENING:
                Event2Post.EventType = LISTENING_MESSAGE;
                break;
        }
